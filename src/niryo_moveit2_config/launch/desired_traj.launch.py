...
def generate_launch_description():
    ...
    # PARTE PER LA TRAIETTORIA DESIDERATA ((parte in più rispetto a niryo_slam.launch.py))
    desired_traj_node = Node(
    package="niryo_moveit2_config",
    executable="desired_trajectory_sampling",
    output="screen",
    parameters=[
        moveit_config.robot_description,
        moveit_config.robot_description_semantic,
        moveit_config.robot_description_kinematics,
        moveit_config.planning_pipelines,
        # moveit_config.joint_limits,  # opzionale ma consigliato
        {"use_sim_time": True},
    ],
    )

    delayed_desired_traj_node = TimerAction(
        period=20.0,                            # aspetta 20 secondi
        actions=[desired_traj_node]
    )
