...
def generate_launch_description():
    ...
    robot_description_file = os.path.join(
        get_package_share_directory('niryo_robot_description'), 'urdf', 'niryo_ned2.urdf.xacro'
    )
    joint_controllers_file = os.path.join(
        get_package_share_directory('niryo_moveit2_config'), 'config', 'ros2_controllers.yaml'
    )
    ...
    moveit_config = (
        ...
        .robot_description(file_path="config/niryo_ned2.urdf.xacro")
    ...
    rviz_node = Node(
        ...
        parameters=[
            ...
            moveit_config.joint_limits,  #aggiunto
        ],
    )

    # Static TF: world → base_link   --Aggiunto
    static_tf = Node(
        package="tf2_ros",
    ...
    controller_manager_node = Node(
        ...
        remappings=[
            ("/controller_manager/robot_description", "/robot_description"),  #-cambiato
        ],
        output='both',  #-cambiato
    )
    ...
    arm_trajectory_controller_spawner = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["niryo_arm_controller", "--controller-manager", "/controller_manager"],
        output="screen",
    )
